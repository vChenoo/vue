...
import {queryOtherCharges, queryCollect} from '@/api/database'
...
export default {
	...
	data () {
        ...
        collect: '0',
			...
			rules: {
				name: [
					{required: true, message: '请输入相关收费项目名称', trigger: 'blur'}
				],
        collect: [
          {required: true, message: '请选择汇总项目', trigger: 'change'}
			...
			tableData: [],  //  表格数据
      collectOption: [],  // 下拉 发票项目
      collectOptSel: [],  // 查询下拉 发票项目
	...
	methods: {
		/* 格式化 */
    formatterCollect (item) { // 发票项目
      let valArr = []
      valArr = this.collectOption.filter(val => {
    ...
    enter (el, done) {
      ...
      queryCollect().then(response => {
        ...
        this.collectOptSel = JSON.parse(JSON.stringify(this.collectOption))
        this.collectOptSel.unshift({code: '0', name: '不限'})
      })
      // 常量查询 是否
